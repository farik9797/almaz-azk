  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <?php foreach ($products as $p): ?>
        <div class="bg-[#F8FAFC] rounded-2xl border-2 border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:border-navy transition-all flex flex-col justify-between">
          ...
          <div class="p-6 pt-0 space-y-2">
            ...
            <a href="tel:<?php echo esc_attr(azk_tel($phone_wholesale)); ?>" class="w-full text-xs text-slate-500 hover:text-navy font-medium text-center py-1 flex items-center justify-center space-x-1"><i data-lucide="file-text" class="w-3.5 h-3.5"></i><span>Запросить Паспорт качества</span></a>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
    ...
    <div class="relative rounded-2xl overflow-hidden border border-slate-200 shadow-md bg-gradient-to-r from-navy via-navy-light to-navy-dark text-white p-6 sm:p-8 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
      <div class="md:col-span-8 space-y-3">
        ...
        <div class="pt-2"><a href="tel:<?php echo esc_attr(azk_tel($phone_wholesale)); ?>" class="bg-accent hover:bg-accent-dark text-navy font-extrabold text-xs px-4 py-2.5 rounded-lg shadow inline-flex items-center space-x-2"><span>Позвонить за паспортами качества</span><i data-lucide="arrow-right" class="w-4 h-4"></i></a></div>
      </div>
    ...
    <div class="bg-navy text-white rounded-2xl p-6 sm:p-8 border border-slate-800 shadow-xl space-y-6" data-calc-products="<?php echo esc_attr(wp_json_encode($calc_data)); ?>">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-5 bg-navy-light rounded-xl p-6 border border-slate-700 flex flex-col justify-between space-y-4">
          ...
          <a href="tel:<?php echo esc_attr(azk_tel($phone_wholesale)); ?>" class="w-full bg-accent hover:bg-accent-dark text-navy font-extrabold py-3.5 px-4 rounded-lg text-sm shadow flex items-center justify-center space-x-2"><span>Оформить заявку на этот объём</span><i data-lucide="arrow-right" class="w-4 h-4"></i></a>
        </div>
      </div>
    </div>
  </div>
